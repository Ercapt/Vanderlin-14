...
public sealed class ZLevelBuildingSystem : EntitySystem
{
    ...
    private bool TryGetAuthoredStationGrid(EntityUid mapUid, Vector2 worldPos, out EntityUid gridUid)
    {
        ...
        var coords = new MapCoordinates(worldPos, mapComp.MapId);
        if (_mapManager.TryFindGridAt(coords, out var foundGrid, out _) && HasComp<StationMemberComponent>(foundGrid))
        {
            gridUid = foundGrid;
            return true;
        }

        if (HasComp<StationMemberComponent>(mapUid) && _gridQuery.HasComponent(mapUid))
        {
            gridUid = mapUid;
            return true;
        }
