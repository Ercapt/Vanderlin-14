...
public abstract partial class CMUSharedZLevelsSystem : EntitySystem
{
    ...
    {
        var uid = Spawn(null);
        var comp = AddComp<CMUZLevelsNetworkComponent>(uid);
        return (uid, comp);
    }

    [PublicAPI]
    public bool TryAddMapsIntoZNetwork(Entity<CMUZLevelsNetworkComponent> network, Dictionary<EntityUid, int> mapsToAdd)
    {
        foreach (var (mapUid, depth) in mapsToAdd)
        {
            network.Comp.ZLevels[depth] = mapUid;
            network.Comp.ZLevelByEntity[mapUid] = depth;

            var zComp = EnsureComp<CMUZLevelMapComponent>(mapUid);
            zComp.NetworkUid = network.Owner;
            zComp.Depth = depth;

            if (network.Comp.ZLevels.TryGetValue(depth + 1, out var mapAbove) && mapAbove.HasValue)
            {
                zComp.MapAbove = mapAbove.Value;
                if (_zMapQuery.TryComp(mapAbove.Value, out var aboveZComp))
                {
                    aboveZComp.MapBelow = mapUid;
                    Dirty(mapAbove.Value, aboveZComp);
                }
            }

            if (network.Comp.ZLevels.TryGetValue(depth - 1, out var mapBelow) && mapBelow.HasValue)
            {
                zComp.MapBelow = mapBelow.Value;
                if (_zMapQuery.TryComp(mapBelow.Value, out var belowZComp))
                {
                    belowZComp.MapAbove = mapUid;
                    Dirty(mapBelow.Value, belowZComp);
                }
            }
